...
Catalusis{public String toString(){return "23";}},
Inhibition{public String toString(){return "24";}},
	...
	private static NumberPropertyDefinition reassignVal(NumberPropertyDefinition prop,String val,boolean isEdit,boolean isVis){
		NumberPropertyDefinition newP=new NumberPropertyDefinition(prop.getName(),new BigDecimal(val),(prop.isEditable()&isEdit),(prop.isVisualisable() | isVis));
		return newP;
	}
...
private LinkObjectType Catalusis;
private LinkObjectType Inhibition;

	
	
	public MetabolicContextAdapterSyntaxService(IContext context) {
...
defineParentingMacromolecule();
	...
	//links
createConsume();createProduce();createActivation();createCatalusis();createInhibition();
	//shape set
...
this.shapeSet.add(this.Macromolecule);
	...
	//link set
this.linkSet.add(this.Consume);this.linkSet.add(this.Produce);this.linkSet.add(this.Activation);this.linkSet.add(this.Catalusis);this.linkSet.add(this.Inhibition);		
	}
...
for (IShapeObjectType child : set) {
  this.rmo.getShapeParentingRules().addChild(child);
}
...
this.Compartment.setURL("http://");
 	...
 	this.Compartment.addProperty(GOTerm);
...
for (IShapeObjectType child : set) {
  this.Compartment.getParentingRules().addChild(child);
	...
	ShapeObjectType getCompartment(){
...
set.addAll(Arrays.asList(new IShapeObjectType[]{this.Compartment, this.Process, this.Compound, this.Macromolecule}));
for (IShapeObjectType child : set) {
  this.Process.getParentingRules().addChild(child);
}

	}

	ShapeObjectType getProcess(){
		return this.Process;
	}
...
this.Compound.setURL("http://");
 	...
 	this.Compound.addProperty(Smiles);
  	IPropertyDefinition IC=reassignVal(getPropIC(),"1.0",true,false);
 	this.Compound.addProperty(IC);
 
}

	private void defineParentingCompound(){
HashSet<IShapeObjectType> set=new HashSet<IShapeObjectType>();
set.addAll(Arrays.asList(new IShapeObjectType[]{this.Process, this.Compound}));
for (IShapeObjectType child : set) {
  this.Compound.getParentingRules().addChild(child);
}

	}

	ShapeObjectType getCompound(){
...
this.Macromolecule.setURL("http://");
 	IPropertyDefinition GOTerm=reassignVal(getPropGOTerm()," ",true,false);
 	this.Macromolecule.addProperty(GOTerm);
...
for (IShapeObjectType child : set) {
  this.Macromolecule.getParentingRules().addChild(child);
	...
	ShapeObjectType getMacromolecule(){
...
set.addAll(Arrays.asList(new IShapeObjectType[]{this.Process}));
for (IShapeObjectType tgt : set) {
  this.Consume.getLinkConnectionRules().addConnection(this.Macromolecule, tgt);
}

}

LinkObjectType getConsume(){
	return this.Consume;
}
...
this.Produce.setName("Consumption Link");
...
tport.getLinkEndDecorator().setDecorator(ArrowheadStyle.TRIANGLE, 20,20);
tport.getTerminusDecorator().setDecoratorType(ShapeType.RECTANGLE);
tport.getTerminusDecorator().setSize(0,0);
int[] ctport=new int[]{255,255,255};
tport.getTerminusDecorator().setColourProperties(ctport[0],ctport[1],ctport[2]);
...
LinkObjectType getActivation(){
	return this.Activation;
}
private void createCatalusis(){
HashSet<IShapeObjectType> set=null;
this.Catalusis= new LinkObjectType(this.context, ObjectTypes.Catalusis);
int[] lc=new int[]{0,0,0};
this.Catalusis.setLineProperty(1, LineStyle.SOLID,lc[0],lc[1],lc[2]);
this.Catalusis.setName("Catalusis Link");
this.Catalusis.setLineColourEditable(true);
this.Catalusis.setLineStyleEditable(true);
this.Catalusis.setLineWidthEditable(true);
LinkEndDefinition sport=this.Catalusis.getLinkSource();
LinkEndDefinition tport=this.Catalusis.getLinkTarget();
sport.setOffset(2);
sport.getLinkEndDecorator().setDecorator(ArrowheadStyle.NONE, 10,10);
sport.getTerminusDecorator().setDecoratorType(ShapeType.RECTANGLE);
sport.getTerminusDecorator().setSize(0,0);
...
//this.Catalusis.setDetailedDescription(detailedDescription);
this.Catalusis.setUrl("http://");
set=new HashSet<IShapeObjectType>();
set.addAll(Arrays.asList(new IShapeObjectType[]{this.Process}));
for (IShapeObjectType tgt : set) {
  this.Catalusis.getLinkConnectionRules().addConnection(this.Compound, tgt);
}

}

LinkObjectType getCatalusis(){
	return this.Catalusis;
}
...
private IPropertyDefinition getPropCID(){
	IPropertyDefinition CID=new TextPropertyDefinition("CID"," ",true,true);
	return CID;
}
